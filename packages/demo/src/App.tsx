...
function App() {
  ...
  return (
    <div className="bg-gray-100 h-screen flex overflow-hidden relative">
      ...
      {/* 用户面板 - 悬浮 */}
      {showUserPanel && (
        <div className={`absolute top-0 left-0 h-full w-64 bg-white shadow-xl z-10 transition-transform duration-300 ease-in-out ${showUserPanel ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      {/* 日志面板 - 悬浮 */}
      {showLogPanel && (
        <div className={`absolute top-0 right-0 h-full w-80 xl:w-[30rem] bg-indigo-100 shadow-xl z-10 transition-transform duration-300 ease-in-out ${showLogPanel ? 'translate-x-0' : 'translate-x-full'}`}>
