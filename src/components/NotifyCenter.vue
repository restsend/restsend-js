...
async function handleListApply() {
    list.value = await client.getKnocks()
    store.setApplicationList(list.value)
    console.log(store.setApplicationList(list.value))
}

async function getGroupApplyList() {
    const params = {}
    const aa = await getGroupApplyList(params)
    console.log(aa)
}
...
onMounted(() => {
    console.log(88888)
    handleListApply()
    /* if(location.href.indexOf('notifications/topics') > -1){
       getGroupApplyList()
     }else{
       handleListApply()
     } */
