<script setup>
  ...
  const navbar = [
    ...
    { href: '/contacts', icon: UserIcon, current: ref(false) },
    { href: '/notifications', icon: BellIcon, current: ref(false) },
    ...
    <div class="flex border-t border-t-gray-100 bg-white px-4  py-1 text-gray-500">
      <ul class="flex w-full">
        <li v-for="item in navbar.slice(0, 3)" :key="item" class="relative w-3/4">
          <component
                  :is="item.icon"
                  class="h-8 w-8 shrink-0 cursor-pointer" :class="[item.current.value === true ? 'text-indigo-600' : 'text-gray-400 hover:text-indigo-600']"
            ...
            <li v-for="item in navbar.slice(0, 3)" :key="item" class="relative w-full">
              <div style="display: flex">
                <component
                        ...
                        @click="selectTab(item)"
                        />
                <span style="color: red;padding: 0 5px;margin-top: 5px" v-if="(item.href.indexOf('notifications') > -1) && appStore.applicationList.length">{{appStore.applicationList.length}}</span>
              </div>
              <!--
